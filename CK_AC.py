...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            lstm_cell = tf.contrib.rnn.BasicLSTMCell(10,state_is_tuple=True)
            ...
            rnn_out = tf.reshape(lstm_outputs, [-1,10])
...
class Worker():
    def __init__(self, game, name, s_size, a_size, trainer, model_path, gloabl_episodes):
    ...
    def work(self,max_episode_length, gamma, sess, coord, saver):
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                while self.env.is_finished() == False:
...
with tf.device("/cpu:0"):
    ...
    for i in range(num_workers):
        workers.append(Worker(React.react(),i,s_size,a_size,trainer,model_path,global_episodes))
